...
namespace Book_Haven_System_Ad.Forms
{
    public partial class frmSales : Form
    {
        ...
        private void PrintReceipt(OrderModel order, List<OrderDetailsModel> orderDetails)
        {
            ...
            // You can use PrintDocument, Crystal Reports, or another printing library

            // Example using MessageBox (replace with actual printing)
            ...
            {
                receipt.AppendLine($"{detail.BookID} - Quantity: {detail.Quantity} - Price: {detail.Price:C}");
            }
        ...
        private void cmbDeliveryType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbOrderMethod.SelectedItem != null)
            {
                ...
                {
                    txtDeliveryAddress.Visible = false; // Hide the combo box if POS is selected
                }
                else if (selectedMethod == "Online")
                {
                    txtDeliveryAddress.Visible = true; // Show the combo box if Online is selected
                }
            }
        }
